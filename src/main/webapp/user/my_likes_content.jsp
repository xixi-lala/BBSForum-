    ...
    <main class="flex-1 min-w-0">
        <div class="bg-white rounded-lg shadow-sm p-6">
            <h2 class="text-lg font-bold text-gray-900 mb-5 flex items-center gap-2">
                <i class="fa fa-thumbs-o-up text-blue-500"></i> 我的点赞
            </h2>

            <c:choose>
                <c:when test="${empty likeList}">
                    <div class="text-center py-12 text-gray-400">
                        <i class="fa fa-thumbs-o-up text-4xl block mb-3 text-gray-300"></i>
                        <p class="text-sm">还没有点赞过任何帖子，去<a href="${pageContext.request.contextPath}/" class="text-blue-500 no-underline">浏览帖子</a>发现精彩内容吧！</p>
                    </div>
                </c:when>
                <c:otherwise>
                    <div class="space-y-3">
                        <c:forEach var="post" items="${likeList}">
                                ...
                                <div class="flex items-start gap-3">
                                    <div class="flex-1 min-w-0">
                                        ...
                                        <div class="flex items-center gap-3 mt-2 text-xs text-gray-400">
                                            ...
                                            <span><i class="fa fa-thumbs-o-up text-blue-500"></i> ${post.likeCount}</span>
                                            <span>点赞于 ${fn:substring(post.likeTime, 0, 16)}</span>
                                        </div>
                                    </div>
                                </div>
                            </div>
